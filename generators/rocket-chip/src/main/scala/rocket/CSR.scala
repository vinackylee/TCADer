...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
  ...
  val ygjk_tval = Bits(INPUT, vaddrBits)
  ...
  val test = new Bundle{
    val out1 = UInt(32.W).asOutput
    val out2 = UInt(32.W).asOutput
    val out3 = UInt(32.W).asOutput
    val out4 = UInt(32.W).asOutput
    val out5 = UInt(32.W).asOutput
    val out6 = UInt(32.W).asOutput
    val out7 = UInt(32.W).asOutput
  }
}
...
// 添加utvec
  ...
  val reg_udeleg = Reg(UInt(width = xLen))
  val reg_uie = Reg(UInt(width = xLen))
  val userint = Wire(Bool())
  val usercause = Wire(Bool())
  val user_blk = Wire(Bool())
  ...
  user_blk := reg_udeleg===UI.DELEG && whichInterrupt===12 && io.ygjk_tval(0)===1.U && (reg_uie=/=UI.UIE || reg_mstatus.prv=/=PRV.U)
//  user_blk := reg_udeleg===UI.DELEG && whichInterrupt===12 && io.ygjk_tval(0)===1.U && (reg_uie=/=UI.UIE)

  io.interrupt := (anyInterrupt && !io.singleStep || reg_singleStepped) && !(reg_debug || io.status.cease) && !user_blk
  io.interrupt_cause := interruptCause
  io.bp := reg_bp take nBreakpoints
  io.pmp := reg_pmp.map(PMP(_))

  io.test.out1 := pending_interrupts
  io.test.out2 := read_mip
  io.test.out3 := d_interrupts
  io.test.out4 := m_interrupts
  io.test.out5 := s_interrupts
  io.test.out6 := anyInterrupt
  io.test.out7 := whichInterrupt
  io.test.out1 := reg_mie


  when(io.rocc_interrupt){
    when(read_mip===0.U){
      printf("read_mip=0\n")
    }
    when(read_mip===4096.U){
      printf("read_mip=4096\n")
    }
    when(read_mip===2048.U){
      printf("read_mip=2048\n")
    }
  }
  ...
  if (usingVM) {
    ...
    read_mapping += CSRs.uscratch -> reg_udeleg
    read_mapping += CSRs.uie -> reg_uie    
  }
  ...
  usercause := cause===UInt(interruptMSB)+12 && io.ygjk_tval(0)===1.U
  userint := reg_udeleg===UI.DELEG && reg_mstatus.prv===PRV.U && usercause
  val notDebugTVec = {
    val base = Mux(userint, read_utvec, Mux(delegate, read_stvec, read_mtvec))
  ...
  when (csr_wen) {
    ...
    if (usingVM) {
      ...
      when (decoded_addr(CSRs.uscratch)) { reg_udeleg := wdata }
      when (decoded_addr(CSRs.uie))      { reg_uie := wdata }
    }
    if (usingUser) {
  ...
  def chooseInterrupt(masksIn: Seq[UInt]): (Bool, UInt) = {
    ...
    val priority = standard ++ nonstandard
