...
{
   // 2^n
   val MT_X     = 0.asUInt(4.W)
   val MT_B     = 0.asUInt(4.W)
   val MT_H     = 1.asUInt(4.W)
   val MT_W     = 2.asUInt(4.W)

   val MT_64    = 3.asUInt(4.W)
   val MT_128   = 4.asUInt(4.W)

   val MT_VLEN  = 6.asUInt(4.W)

   val MT_BU    = 8.asUInt(4.W)
   val MT_HU    = 9.asUInt(4.W)
   val MT_WU    = 10.asUInt(4.W)
   val MT_64U   = 11.asUInt(4.W)
   val MT_128U  = 12.asUInt(4.W)

   val M_X   = "b00".asUInt(2.W)
   val M_XRD = "b00".asUInt(2.W) // int load
   val M_XWR = "b01".asUInt(2.W) // int store
   val M_VRD = "b10".asUInt(2.W) // vec load
   val M_VWR = "b11".asUInt(2.W) // vec store
...
class d2h2i1(val addrWidth : Int) extends Bundle{
   val dataInstr = Vec(2,new  Rport(addrWidth, 512))
   val hw = new  Wport(addrWidth, 32)
   val dw = new  Wport(addrWidth, 512)
...
class AsyncScratchPadMemory(val num_core_ports: Int,val num_bytes: Int = (1 << 21))(implicit val conf: CPUConfig) extends Module
{
   ...
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = conf.vlen)) )
   ...
   // scala and vector READ valid and addr
   ...
   when (io.core_ports(DPORT).req.valid && (io.core_ports(DPORT).req.bits.fcn === M_XWR))
   {
      ...
      async_data.io.dw.data := io.core_ports(DPORT).req.bits.data << (req_addri(5,0) << 3)
      async_data.io.dw.addr := Cat(req_addri(31,6), 0.asUInt(6.W))
      async_data.io.dw.mask :=  Mux(req_typi === MT_B, 1.U << req_addri(5,0),
                                Mux(req_typi === MT_H, 3.U << req_addri(5,0), 15.U << req_addri(5,0)))
   }

   // vector WRITE
   when (io.core_ports(DPORT).req.valid && (io.core_ports(DPORT).req.bits.fcn === M_VWR))
   {
      async_data.io.dw.en   := true.B
      async_data.io.dw.data := io.core_ports(DPORT).req.bits.data
      async_data.io.dw.addr := req_addri
      async_data.io.dw.mask := Fill(conf.vlen / 8, 1.U)
      // async_data.io.dw.mask := io.core_ports(DPORT).req.bits.vstore_mask

      // debug
      // printf("\naddr %x\ndata %x\n", req_addri, io.core_ports(DPORT).req.bits.data)
   }
   /////////////////

   ///////////// IPORT
   // scala READ resp data
   if (num_core_ports == 2){
