...
package common
import chisel3._
import chisel3.util.{Cat, Mux1H, PopCount, Fill}
...
class VType extends Bundle {
  //向量协处理器
  val vill    = UInt(1.W)
  val no_name = UInt(23.W)
  val vma     = UInt(1.W)
  val vta     = UInt(1.W)
  val vlmul_f = UInt(1.W)
  val vsew    = UInt(3.W)
  val vlmul_nf= UInt(2.W)
}
...
class CSRFileIO(implicit val conf: CPUConfig) extends Bundle {
  ...
  val vl    = Output(UInt(conf.xprlen.W))
...
class CSRFile(implicit conf: CPUConfig) extends Module
{
  ...
  //向量处理器
  val reset_vtype = WireInit(0.U.asTypeOf(new VType()))
  reset_vtype.vsew := 2.U(3.W) //010->32
  val reg_vtype   = RegInit(reset_vtype)
  val reg_vl      = RegInit(0.U(conf.xprlen.W))
  io.vtype       := reg_vtype
  io.vl          := reg_vl
  ...
  when (wen) {
    ...
    //vcsr寄存器写入
    when (decoded_addr(CSRs.vl))      {  
      reg_vl    := Cat(Fill(16, 0.U), wdata(15, 0))
      val vtype_data = Cat(Fill(24, 0.U), wdata(23, 16))
      val new_vtype = vtype_data.asTypeOf(new VType)
      reg_vtype := new_vtype
    }
